...
TRACKING_WINDOW_PROPERTIES = {
    ...
    "warning_name": "Highlight:TrackingMonitor",
...
FUEL_WINDOW_PROPERTIES = {
    ...
    "color": FUEL_COLOR,
    "warning_name": "Highlight:FuelMonitor",
...
SYSTEM_WINDOW_PROPERTIES = {
    ...
    "color": SYSTEM_COLOR,
    "warning_name": "Highlight:SystemMonitor",
    "data_fn": lambda dataset: get_system_monitor_task_data(dataset),
}
ALL_WINDOW_PROPERTIES = {
    "system": SYSTEM_WINDOW_PROPERTIES,
    "fuel": FUEL_WINDOW_PROPERTIES,
    "tracking": TRACKING_WINDOW_PROPERTIES,
}
...
def get_clicks(line_data):
    line_data = LineData.findall_from_src(
        line_data, "Canvas"
    )  # will contain all click events
    return {
        k: np.array(LineData.pack_variables(v, "timestamp"))
        for k, v in LineData.groupby_dst(line_data)
    }
...
def compute_time_intervals(binary, timestamps, start_time, finish_time, pad="finish"):
    ...
    timestamps[-1] = (
        finish_time if pad == "finish" else timestamps[-2]
    ...
    return SimpleNamespace(
        **dict(
            intervals=ts,
            proportion=df.sum() / (finish_time - start_time),
...
def get_clean_datasets(force=False, n=None, eyetracking=True):
    ...
    if eyetracking:
        datasets = {
            ...
            if LineData.contains_src(data, EYETRACKER_NAME)
        }
    datasets = list(
        sorted(
            [(k.split(".")[0], v) for k, v in datasets.items()], key=lambda x: x[0][:3]
        )
    )
    datasets = {
        k: dict(g) for k, g in itertools.groupby(datasets, key=lambda x: x[0][:3])
    }
    ...
    for file in tqdm(itertools.islice(files, n), desc="loading files..."):
        ...
        for i, line in enumerate(dataset[file.name]):
            if line.event_src == EYETRACKER_NAME:  # first occurance of eyetracker data.
                ...
                eye_1prev_timestamp = dataset[file.name][
                    i - 1
                ].timestamp  # i should not be 0...
                break
        for line in LineData.findall_from_src(dataset[file.name], EYETRACKER_NAME):
            line.timestamp = (
                line.timestamp - eye_first_timestamp
            ) + eye_1prev_timestamp
    return dataset
...
def get_system_monitor_task_data(line_data):
    ...
    wl_data = [
        np.array(
            LineData.pack_variables(
                LineData.findall_from_src(line_data, wl), "timestamp", "value"
            )
        )
        for wl in WARNINGLIGHT_NAMES
    ]
    sc_data = [
        np.array(
            LineData.pack_variables(
                LineData.findall_from_src(line_data, sc), "timestamp", "value"
            )
        )
    ...
    return SimpleNamespace(
        **dict(
            components={
                k: pd.DataFrame(
                    dict(timestamp=v[:, 0], value=v[:, 1], failure=v[:, 2].astype(bool))
                )
                for k, v in zip(WARNINGLIGHT_NAMES + SCALE_NAMES, data)
            },
...
def get_tracking_task_data(line_data, l2_threshold=50):
    ...
    return SimpleNamespace(
        components={TARGET_NAME: df}, start_time=start_time, finish_time=finish_time
    )
...
def get_fuel_task_data(line_data):
    ...
    def _get_data(src, line_data):
        ...
        return pd.DataFrame(
            dict(timestamp=data[:, 0], failure=1 - data[:, 1].astype(bool))
        )

    tank_data = {src: _get_data(src, line_data) for src in FUEL_TANK_NAMES}
    return SimpleNamespace(
        components=tank_data, start_time=start_time, finish_time=finish_time
    )
    ...
    def from_line(cls, line):
        ...
        if match:
            ...
            variables = match.group(
                4
            )  # this group is a bit dodgy because of the 'cause' variable... it should be removed.
            variables = variables.split("'cause'")[
                0
            ]  # this assumes that 'cause' is at the end... TODO CHECK THIS! its very annoying to process it otherwise
...
def _get_dataset(force=False):
    ...
    clean_directory = pathlib.Path(
        directory, DATA_NAME.split(".")[0]
    )  # where data will end up
    ...
    if force or len([f for f in clean_directory.iterdir()]) == 0:
        ...
        files = [
            (f, clean_file_name(f))
            for f in pathlib.Path(data_directory).iterdir()
            if f.name.endswith(".txt") and "event_log" not in f.name
        ]
        ...
        shutil.copyfile(
            pathlib.Path(old_directory, "A_config.json"),
            pathlib.Path(clean_directory, "A_config.json"),
        )
        shutil.copyfile(
            pathlib.Path(old_directory, "B_config.json"),
            pathlib.Path(clean_directory, "B_config.json"),
        )
        demo_name = "Participant Demographics (red shoes-cockpit).xlsx"
        shutil.copyfile(
            pathlib.Path(old_directory, demo_name),
            pathlib.Path(clean_directory, "demographics.xlsx"),
        )
...
def get_warning_data(dataset):
    ...
    # dataframes instead?
    return {
        k: get_data_from_source(v["warning_name"])
...
def get_task_data(dataset):
    return {
        task: properties["data_fn"](dataset).components
...
def get_demographics_data(eyetracking=False, ignore=["P00", "P03"]):
    ...
    valid_participants = load_nested_dict(
        "data/Processed", eyetracking=eyetracking, ignore=ignore
    ).keys()
